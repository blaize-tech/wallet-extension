...
async function start () {
  ...
  if (versionAlreadyExists) {
    ...
    await exec(`sentry-cli releases --org 'affilcoin' --project 'affilcoin' new ${VERSION}`)
    console.log(`removing any existing files from Sentry release "${VERSION}"...`)
    await exec(`sentry-cli releases --org 'affilcoin' --project 'affilcoin' files ${VERSION} delete --all`)
  }
  ...
  if (!versionHasArtifacts) {
    ...
    await exec(`for FILEPATH in ./dist/chrome/*.js; do [ -e $FILEPATH ] || continue; export FILE=\`basename $FILEPATH\` && echo uploading $FILE && sentry-cli releases --org 'affilcoin' --project 'affilcoin' files ${VERSION} upload $FILEPATH affilcoin/$FILE; done;`)
    console.log(`uploading sourcemaps Sentry release "${VERSION}"...`)
    await exec(`sentry-cli releases --org 'affilcoin' --project 'affilcoin' files ${VERSION} upload-sourcemaps ./dist/sourcemaps/ --url-prefix 'sourcemaps'`)
...
async function checkIfAuthWorks () {
  const itWorked = await doesNotFail(async () => {
    await exec(`sentry-cli releases --org 'affilcoin' --project 'affilcoin' list`)
  })
  return itWorked
}
...
async function checkIfVersionExists () {
  const versionAlreadyExists = await doesNotFail(async () => {
    await exec(`sentry-cli releases --org 'affilcoin' --project 'affilcoin' info ${VERSION}`)
  })
  return versionAlreadyExists
}

async function checkIfVersionHasArtifacts () {
  const artifacts = await exec(`sentry-cli releases --org 'affilcoin' --project 'affilcoin' files ${VERSION} list`)
