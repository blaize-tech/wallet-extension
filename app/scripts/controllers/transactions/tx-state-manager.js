...
class TransactionStateManager extends EventEmitter {
  ...
  */
  generateTxMeta (opts) {
    ...
    if (netId === 'loading') throw new Error('Affilcoin is having trouble connecting to the network')
    return extend({
      id: createId(),
      time: (new Date()).getTime(),
      status: 'unapproved',
      affilcoinNetworkId: netId,
  ...
  */
  getTxList () {
    ...
    return fullTxList.filter((txMeta) => txMeta.affilcoinNetworkId === network)
  }
  ...
  */
  setTxStatusFailed (txId, err) {
    const error = !err ? new Error('Internal affilcoin failure') : err
  ...
  */
  wipeTransactions (address) {
    ...
    const otherAccountTxs = txs.filter((txMeta) => !(txMeta.txParams.from === address && txMeta.affilcoinNetworkId === network))
