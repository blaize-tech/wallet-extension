...
When publishing a new version of Affilcoin, we follow this procedure:

## Overview

The below diagram outlines our process for design, development, and release. Building Affilcoin is a community affair, and many steps of the process invite participation from external contributors as indicated. All QA, code review, and release of new versions is done by members of the core Affilcoin team.
...
With each pull request, the @AffilcoinBot will comment with a build of that new pull request, so after bumping the version on `develop`, open a pull request against `master`, and once the pull request is reviewed and merged, you can download those builds for publication.
...
3. Publish to [firefox addon marketplace](http://addons.mozilla.org/en-us/firefox/addon/ether-affilcoin).
4. Publish to [Opera store](https://addons.opera.com/en/extensions/details/affilcoin/).
5. Post on [Github releases](https://github.com/Affilcoin/affilcoin-extension/releases) page.
...
The version and changelog bump should then be made off the `master` branch, and then merged to `develop` to bring the two branches back into sync. Further time can be saved by incorporating the version/changelog bump into the PR against `master`, since we rely on @AffilcoinBot to run tests before merging.
