...
describe('TransactionStateManager', function () {
  ...
  describe('#setTxStatusSigned', function () {
    it('sets the tx status to signed', function () {
      const tx = { id: 1, status: 'unapproved', affilcoinNetworkId: currentNetworkId, txParams: {} }
  ...
  describe('#addTx', function () {
    it('adds a tx returned in getTxList', function () {
      const tx = { id: 1, status: 'confirmed', affilcoinNetworkId: currentNetworkId, txParams: {} }
      txStateManager.addTx(tx, noop)
      const result = txStateManager.getTxList()
      assert.ok(Array.isArray(result))
      assert.equal(result.length, 1)
    ...
    it('does not override txs from other networks', function () {
      const tx = { id: 1, status: 'confirmed', affilcoinNetworkId: currentNetworkId, txParams: {} }
      const tx2 = { id: 2, status: 'confirmed', affilcoinNetworkId: otherNetworkId, txParams: {} }
      txStateManager.addTx(tx, noop)
    ...
    it('cuts off early txs beyond a limit', function () {
      ...
      for (let i = 0; i < limit + 1; i++) {
        const tx = { id: i, time: new Date(), status: 'confirmed', affilcoinNetworkId: currentNetworkId, txParams: {} }
        txStateManager.addTx(tx, noop)
      }
      const result = txStateManager.getTxList()
    ...
    it('cuts off early txs beyond a limit whether or not it is confirmed or rejected', function () {
      const limit = txStateManager.txHistoryLimit
      for (let i = 0; i < limit + 1; i++) {
        const tx = { id: i, time: new Date(), status: 'rejected', affilcoinNetworkId: currentNetworkId, txParams: {} }
        txStateManager.addTx(tx, noop)
      }
      const result = txStateManager.getTxList()
      assert.equal(result.length, limit, `limit of ${limit} txs enforced`)
    ...
    it('cuts off early txs beyond a limit but does not cut unapproved txs', function () {
      const unconfirmedTx = { id: 0, time: new Date(), status: 'unapproved', affilcoinNetworkId: currentNetworkId, txParams: {} }
  ...
  describe('#updateTx', function () {
    it('replaces the tx with the same id', function () {
      txStateManager.addTx({ id: '1', status: 'unapproved', affilcoinNetworkId: currentNetworkId, txParams: {} }, noop)
      txStateManager.addTx({ id: '2', status: 'confirmed', affilcoinNetworkId: currentNetworkId, txParams: {} }, noop)
    ...
    it('throws error and does not update tx if txParams are invalid', function () {
      ...
      txStateManager.addTx({ id: 1, status: 'unapproved', affilcoinNetworkId: currentNetworkId, txParams: validTxParams })
  ...
  describe('#getFilteredTxList', function () {
    it('returns a tx with the requested data', function () {
      const txMetas = [
        { id: 0, status: 'unapproved', txParams: { from: '0xaa', to: '0xbb' }, affilcoinNetworkId: currentNetworkId },
        { id: 1, status: 'unapproved', txParams: { from: '0xaa', to: '0xbb' }, affilcoinNetworkId: currentNetworkId },
        { id: 2, status: 'unapproved', txParams: { from: '0xaa', to: '0xbb' }, affilcoinNetworkId: currentNetworkId },
        { id: 3, status: 'unapproved', txParams: { from: '0xbb', to: '0xaa' }, affilcoinNetworkId: currentNetworkId },
        { id: 4, status: 'unapproved', txParams: { from: '0xbb', to: '0xaa' }, affilcoinNetworkId: currentNetworkId },
        { id: 5, status: 'confirmed', txParams: { from: '0xaa', to: '0xbb' }, affilcoinNetworkId: currentNetworkId },
        { id: 6, status: 'confirmed', txParams: { from: '0xaa', to: '0xbb' }, affilcoinNetworkId: currentNetworkId },
        { id: 7, status: 'confirmed', txParams: { from: '0xbb', to: '0xaa' }, affilcoinNetworkId: currentNetworkId },
        { id: 8, status: 'confirmed', txParams: { from: '0xbb', to: '0xaa' }, affilcoinNetworkId: currentNetworkId },
        { id: 9, status: 'confirmed', txParams: { from: '0xbb', to: '0xaa' }, affilcoinNetworkId: currentNetworkId },
      ]
  ...
  describe('#wipeTransactions', function () {
    ...
    it('should remove only the transactions from a specific address', function () {
      ...
      const txMetas = [
        { id: 0, status: 'unapproved', txParams: { from: specificAddress, to: otherAddress }, affilcoinNetworkId: currentNetworkId },
        { id: 1, status: 'confirmed', txParams: { from: otherAddress, to: specificAddress }, affilcoinNetworkId: currentNetworkId },
        { id: 2, status: 'confirmed', txParams: { from: otherAddress, to: specificAddress }, affilcoinNetworkId: currentNetworkId },
      ]
      txMetas.forEach((txMeta) => txStateManager.addTx(txMeta, noop))
    ...
    it('should not remove the transactions from other networks', function () {
      const txMetas = [
        { id: 0, status: 'unapproved', txParams: { from: specificAddress, to: otherAddress }, affilcoinNetworkId: currentNetworkId },
        { id: 1, status: 'confirmed', txParams: { from: specificAddress, to: otherAddress }, affilcoinNetworkId: otherNetworkId },
        { id: 2, status: 'confirmed', txParams: { from: specificAddress, to: otherAddress }, affilcoinNetworkId: otherNetworkId },
      ]
      ...
      const txFromOtherNetworks = txStateManager.getFullTxList().filter((txMeta) => txMeta.affilcoinNetworkId === otherNetworkId)
